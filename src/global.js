...
() => {
    function hasIncompleteEncounters(encounters, imports, schedule, enrolmentBaseDateConcept) {
        ...
        const numberOfEncountersExpected = schedule.filter(days => days <= daysBetween).length;
        return encounters.length < numberOfEncountersExpected;
    }
